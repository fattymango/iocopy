...
func (l *LinuxInputCapture) Capture(eventCh chan<- model.InputEvent, stopCh <-chan struct{}) error {
...
func (l *LinuxInputCapture) captureKeyboard(deviceID string, eventCh chan<- model.InputEvent, stopCh <-chan struct{}) {
...
func (l *LinuxInputCapture) captureMouse(deviceID string, eventCh chan<- model.InputEvent, stopCh <-chan struct{}) {
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "EVENT") {
			...
			if err == nil && (x != lastX || y != lastY) {
				moveEvent := model.MouseMoveEvent{X: x, Y: y}
